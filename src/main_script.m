%% To do:
%% - generate a file of out-of-facility births and read it in
...
%% This allows us to specify certain countries.
%% This can either be hard-coded here or specified in a text file (the latter can be auto-generated by the job submission script if running on the South Ken HPC.
%% if running on the DIDE windows cluster, talk to Wes.
%% TUTAJ:
...
%% MP: currently there are no sensitivity analyses.
%% MP: scenarios are dealt with in country_level_analyses.m.
%TUTAJ:
sensitivity_analysis_list = {'default'};
...
%% Now model states. Note that these aren't actually used in the model code - I think in the figure drawing matlab script.
Snames = {
    'Susceptible', ... % 1
...
CFR_Acute = 0.05; % proportion of people with severe acute infection (15) that die from severe acute infection
rate_6months = 2; % the rate necessary to move everyone out of acute states (14 and 15) into State 2 (Immune Tolerant) in 6 months
...
Prog(7, 11) = 0.30; % Decomp Cirrhosis to HBV death.

% Fill-in transitions from Liver Cancer:
%% WADA:
%% This is now done in country_leel_analyses.m.
%%Prog(8, 11) = params.CancerDeathRate;  % HCC to HBV death.
...
for country_num = 1:num_countries
    ...
    %% MAP and CPAD efficacies when SAg+ or EAg+
    temp.efficacy_MAP_HbSAg = 0.85;      %% Placeholder assumption
    temp.efficacy_MAP_HbEAg = 0.7;      %% Placeholder assumption
    temp.efficacy_CPAD_HbSAg = 0.9;    %% Placeholder assumption
    temp.efficacy_CPAD_HbEAg = 0.75;    %% Placeholder assumption

    %% Now get rid of these (previously this was done in country_level_analyses.m) - they are overwritten later on.
    temp = rmfield(temp,'Efficacy_BirthDoseVacc_HbEAg'); %% Now set in country_level_analyses.m as stochas_params_mat(stochas_run_num,end-1).
    temp = rmfield(temp,'Efficacy_InfantVacc'); %% Now set in country_level_analyses.m as stochas_params_mat(stochas_run_num,end).
